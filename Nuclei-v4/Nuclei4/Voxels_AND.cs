...
namespace Nuclei4
{
    public class Voxels_AND : GH_Component, IGH_VariableParameterComponent
    {
        ...
        /// <summary>
        /// Initializes a new instance of the Voxels_AND class.
        /// </summary>
        public Voxels_AND()
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            //make a list of the areas that intersect
            ConcurrentBag<Voxel> activeVoxelsConcurrent = new ConcurrentBag<Voxel>();
            int[,,] _counter = new int[resX, resY, resZ];
            int activeParamCount = 0;

            //count the number of coinciding voxels
                    ...
                    Parallel.For(0, resX, i =>
                    {
                        for (int j = 0; j < resY; j++)
                        {
                            for (int k = 0; k < resZ; k++)
                            {
                                ...
                                {
                                    _counter[i, j, k]++;
                                    if (activeParamCount < _counter[i, j, k]) activeParamCount = _counter[i, j, k];
                                }
                            }
                        }
                    }
            ...
            List<Voxel> activeVoxels = activeVoxelsConcurrent.ToList();
                            ...
                            for (int j = 0; j < resY; j++)
                            {
                                for (int k = 0; k < resZ; k++)
                                {
                                    ...
                                    if (inputVoxels[i, j, k] != null)
                                    {
                                        ...
                                        if (_counter[i, j, k] == activeParamCount)
                                        {
        ...
        bool trySolveWithSidecar(IGH_DataAccess DA)
        {
            ...
            VoxelField firstField = null;
            VoxelGridData first = null;

            for (int p = 0; p < Params.Input.Count; p++)
            {
                VoxelField currentField;
                if (!VoxelFieldAccess.TryGet(DA, p, Globals.voxelSize, out currentField))
                {
                    continue;
                }

                VoxelGridData data = currentField.Data;
                if (first == null)
                {
            ...
            if (canReuseCachedSidecarOutput(inputs, mergeMode))
            {
            ...
            VoxelField outputField = firstField.WithData(outputData);
            cacheSidecarOutput(inputs, mergeMode, outputField, outputData);
        ...
        bool canReuseCachedSidecarOutput(List<VoxelGridData> inputs, VoxelGridMergeMode mergeMode)
        {
            if (cachedSidecarOutputField == null ||
        ...
        void cacheSidecarOutput(List<VoxelGridData> inputs, VoxelGridMergeMode mergeMode, VoxelField outputField, VoxelGridData outputData)
        {
