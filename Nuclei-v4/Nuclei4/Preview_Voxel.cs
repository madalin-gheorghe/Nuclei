    ...
    public class Preview_Voxel : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            if (Params.Input[0].SourceCount != 0)
            {
                ...
                if (VoxelFieldAccess.TryGet(DA, "Voxels", Globals.voxelSize, out inputField))
                {
                    voxel = inputField;
                }
                else
        ...
        void updateClippingBox()
        {
            ...
            if (voxel != null)
            {
                double size = voxel.VoxelSize > 0 ? voxel.VoxelSize : voxelSize;
                clippingBox = new BoundingBox(
                    new Point3d(0, 0, 0),
                    new Point3d(voxel.ResX * size, voxel.ResY * size, voxel.ResZ * size));
        ...
        bool tryReuseStaticPreviewCache(int resX, int resY, int resZ)
        {
            ...
            if (!ReferenceEquals(staticPreviewCacheField, voxel))
            {
                return false;
            }

            VoxelGridData currentData = voxel.Data;
        ...
        bool tryGetPreviewValueAt(VoxelGridData previewData, int x, int y, int z, out double value)
        {
            ...
            {
                case 0:
                    value = previewData != null ? previewData.MinimumDensity.Get(flatIndex) : -1;
                    if (value < 0) return false;
                    break;
                case 1:
                    value = previewData != null ? previewData.MaximumDensity.Get(flatIndex) : -1;
                    if (value < 0) return false;
                    break;
                case 2:
                    value = previewData != null ? previewData.Speed.Get(flatIndex) : -1;
                    if (value < 0) return false;
                    break;
                case 3:
                    value = previewData != null ? previewData.SensorDistance.Get(flatIndex) : -1;
                    if (value < 0) return false;
                    break;
                case 4:
                    value = previewData != null ? previewData.SensorAngle.Get(flatIndex) : -1;
                    if (value < 0) return false;
                    break;
                case 5:
                    value = previewData != null ? previewData.RotationAngle.Get(flatIndex) : -1;
                    if (value < 0) return false;
                    break;
                case 6:
                    ...
                    break;
                case 7:
                    value = voxel != null ? voxel.GetScalarValue(VoxelPreviewField.SlimeChemoattractants, flatIndex) : previewData != null ? previewData.Density.Get(flatIndex) : 0;
                    if (value <= 0.01) return false;
                    break;
                case 8:
                    if (voxel == null) return false;
                    value = voxel.GetScalarValue(VoxelPreviewField.AntFoodPheromones, flatIndex);
                    if (value <= 0.01) return false;
                    break;
                case 9:
                    if (voxel == null) return false;
                    value = voxel.GetScalarValue(VoxelPreviewField.AntBasePheromones, flatIndex);
                    if (value <= 0.01) return false;
                    break;
                default:
                    return false;
        ...
        VoxelField staticPreviewCacheField;
