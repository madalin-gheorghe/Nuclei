...
namespace Nuclei3
{
    public class Voxel_MeshInclusion : GH_Component
    {
        ...
        bool trySolveWithSidecar(IGH_DataAccess DA)
        {
            ...
            VoxelSelectionBuilder insideSelection = new VoxelSelectionBuilder(inputData.Count);
            ...
            if (invert) insideSelection.Invert();
            insideSelection.IntersectWith(inputData);
            VoxelGridData outputData = insideSelection.ApplyTo(inputData);
            ...
            for (int ordinal = 0; ordinal < outputData.ActiveCount; ordinal++)
            {
                ...
                if (!invert)
                {
                    Point3d point = outputData.CenterPoint(flatIndex);
                    for (int meshIndex = 0; meshIndex < meshes.Count; meshIndex++)
                    {
                        Mesh mesh = meshes[meshIndex];
